...
class WooInstanceConfig(models.TransientModel):
    ...
    def woo_test_connection(self):
        ...
        self.woo_create_analytic_plan_account(instance)
        ...
        if self._context.get('is_calling_from_onboarding_panel', False):
            ...
            self.env['onboarding.onboarding.step'].sudo().action_validate_step(
                "woo_commerce_ept.onboarding_woo_instance_configuration")
    ...
    def woo_create_analytic_plan_account(self, woo_instance):
        """
        This method is use to create analytic plan and account
        @author: Nilam Kubavat on Date 08-Nov-2023.
        """
        if self.env.user.has_group('analytic.group_analytic_accounting') and woo_instance:
            analytic_account_plan = self.woo_search_or_create_analytic_account_plan()
            analytic_account_id = self.woo_search_or_create_analytic_account(woo_instance.name, analytic_account_plan)
            woo_instance.write({'woo_analytic_account_id': analytic_account_id.id})
        return True

    def woo_search_or_create_analytic_account_plan(self):
        """
        ...
        analytic_account_plan = analytic_account_plan_obj.search([('name', '=', 'woo_commerce')], limit=1)
        if not analytic_account_plan:
            analytic_account_plan = analytic_account_plan_obj.create({'name': 'woo_commerce'})
        return analytic_account_plan

    def woo_search_or_create_analytic_account(self, account_name, analytic_account_plan):
        """
...
class ResConfigSettings(models.TransientModel):
    ...
    woo_user_ids = fields.Many2many('res.users', 'woo_res_config_settings_res_users_rel_ept', 'res_config_settings_id',
                                    'res_users_id',
    ...
    woo_stock_export_warehouse_ids = fields.Many2many("stock.warehouse",
                                                      "woo_res_config_settings_stock_warehouse_rel_ept",
                                                      "res_config_settings_id", "stock_warehouse_id",
    ...
    is_woo_order_shipped = fields.Boolean("Create a shipment of order from webhook",
                                          help="If checked, it will create a shipment")
    is_force_done_picking = fields.Boolean("Force Done picking from webhook orders",
                                           help="If checked, it will force done picking if stock available or not available ")
    is_create_woo_order_refund = fields.Boolean("Create a Refund of order from webhook",
                                                help="If checked, it will create a refund")
    ...
    def execute(self):
        ...
        if instance:
            ...
            values['woo_ship_order_webhook'] = self.woo_ship_order_webhook if self.create_woo_order_webhook else False
    ...
    def woo_save_basic_configurations(self):
        ...
        if instance:
            ...
            company = instance.company_id
            self.env['onboarding.onboarding.step'].sudo().action_validate_step(
                "woo_commerce_ept.onboarding_woo_basic_configuration")
        return True
    ...
    def woo_save_financial_status_configurations(self):
        ...
        if instance:
            ...
            company = instance.company_id
            self.env['onboarding.onboarding.step'].sudo().action_validate_step(
                "woo_commerce_ept.onboarding_woo_financial_status_configuration")
    ...
    def onchange_analytic_accounting(self):
        res_config_woo_commerce_instance = self.env['res.config.woo.instance']
        woo_instance_obj = self.env['woo.instance.ept']
        ...
        if self.group_analytic_accounting:
            ...
            for instance in instances:
                res_config_woo_commerce_instance.woo_create_analytic_plan_account(instance)
        return res
